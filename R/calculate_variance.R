...
#'    head(var$variance, 5)
calculate_variance = function(mat, clinical_data, p.adjust.method="fdr", cores=1){
  ...
  if(length(unique(clinical_data[,i]))>2){
   ...
   # attributes(tmp)= NULL
    ...
    tmp= df  |>
      ...
      group_by(features) |>
      select_at(vars(features, value, i))|>
      rstatix::wilcox_test(formula= as.formula(paste("value~", i)), p.adjust.method = p.adjust.method) |>
      as.data.frame() |>
